...
contactform.addEventListener('submit', e => {
    ...
    db.collection('contacts').add(contact).then(() => {
        ...
        contactform.querySelector('.error').textContent = err.massege
    })
})
...
editform.addEventListener('submit', e => {
    ...
    db.collection('contacts').doc(updateId).update(contact).then(() => {
        ...
        editform.querySelector('.error').textContent = err.massege
    })
})
...
db.collection('contacts').onSnapshot(snapshot => {
    (snapshot.docChanges()).forEach(change => {
        ...
        if (change.type === 'modified') {
            updateContacts(change.doc.data(), change.doc.id)
        }
    });
})
// Snapshot is used when user will edit some changes in web when
// he is offline when he comes back online then if changes detected
// then it will send to firebase
